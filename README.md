...
- 即時資料整合：串接天氣與國定假日公開 API，建立延遲估算基礎
- 跨時區判斷：依出發地與目的地時區分析實際作業日與假期風險
- 多因子風險評估：綜合天氣、季節、節假日與航線咽喉點計算延遲
- 空海運模式切換：依不同運輸型態套用不同門檻與風險邏輯
- 輕量後端封裝：透過 Python 後端統一代理外部資料來源與快取行為
- 啟動自動化：提供批次檔與 PowerShell 腳本，快速啟動前後端流程

延伸文件：
- 產品規格版 skills 文件：[SKILLS.md](/abs/path/c:/Users/btohr/Desktop/AI_text/SKILLS.md)
- AI agent skill 定義：[skills/freightcast-delay-analysis/SKILL.md](/abs/path/c:/Users/btohr/Desktop/AI_text/skills/freightcast-delay-analysis/SKILL.md)
